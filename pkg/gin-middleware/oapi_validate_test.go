...
func TestOapiRequestValidator(t *testing.T) {
	...
	{
		rec := doGet(t, g, "http://not.gerhardwagner.ai/resource")
	...
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/resource")
	...
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/resource?id=500")
	...
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/resource?id=foo")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}
	...
	// Send a good request body
	{
		...
		rec := doPost(t, g, "http://gerhardwagner.ai/resource", body)
	...
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/protected_resource")
		assert.Equal(t, http.StatusNoContent, rec.Code)
		assert.True(t, called, "Handler should have been called")
		called = false
	}
	...
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/protected_resource2")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}
	...
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/protected_resource_401")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
...
func TestOapiRequestValidatorWithOptionsMultiError(t *testing.T) {
	...
	// Let's send a good request, it should pass
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/multiparamresource?id=50&id2=50")
		assert.Equal(t, http.StatusOK, rec.Code)
		assert.True(t, called, "Handler should have been called")
		called = false
	}
	...
	// a bad status
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/multiparamresource?id=50")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
	...
	// a bad status
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/multiparamresource")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		body, err := io.ReadAll(rec.Body)
		if assert.NoError(t, err) {
			assert.Contains(t, string(body), "multiple errors encountered")
	...
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/multiparamresource?id=500")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		body, err := io.ReadAll(rec.Body)
		if assert.NoError(t, err) {
			assert.Contains(t, string(body), "multiple errors encountered")
	...
	// return a bad status
	{
		rec := doGet(t, g, "http://gerhardwagner.ai/multiparamresource?id=abc&id2=1")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		body, err := io.ReadAll(rec.Body)
		if assert.NoError(t, err) {
			assert.Contains(t, string(body), "multiple errors encountered")
