...
func TestRefPathToGoType(t *testing.T) {
	...
	globalState.importMapping = constructImportMapping(map[string]string{
		...
		"http://gerhardwagner.com/doc.json": "externalref1",
	})
	...
	tests := []struct {
		...
		{
			name:   "url-root",
			path:   "http://gerhardwagner.com/doc.json#/foo_bar",
			goType: "externalRef1.FooBar",
		},
		{
			name:   "url-pathed",
			path:   "http://gerhardwagner.com/doc.json#/components/parameters/foo_bar",
			goType: "externalRef1.FooBar",
		},
		{
			name: "local-too-deep",
		...
		{
			name: "url-too-deep",
			path: "http://gerhardwagner.com/doc.json#/components/parameters/foo/foo_bar",
		},
	}
...
func TestIsWholeDocumentReference(t *testing.T) {
	...
	assert.Equal(t, true, IsWholeDocumentReference("../doc.json"))
	assert.Equal(t, false, IsWholeDocumentReference("http://gerhardwagner.com/doc.json#/components/parameters/foo_bar"))
	assert.Equal(t, true, IsWholeDocumentReference("http://gerhardwagner.com/doc.json"))
}
...
func TestIsGoTypeReference(t *testing.T) {
	...
	assert.Equal(t, false, IsGoTypeReference("../doc.json"))
	assert.Equal(t, true, IsGoTypeReference("http://gerhardwagner.com/doc.json#/components/parameters/foo_bar"))
	assert.Equal(t, false, IsGoTypeReference("http://gerhardwagner.com/doc.json"))
}
...
func TestRefPathToObjName(t *testing.T) {
	...
	for in, want := range map[string]string{
		...
		"http://gerhardwagner.com/schemas/document.json#/objObj": "objObj",
	} {
