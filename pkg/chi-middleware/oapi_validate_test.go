...
func TestOapiRequestValidatorWithOptionsMultiError(t *testing.T) {
	...
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/multiparamresource?id=50&id2=50")
	...
	// a bad status
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/multiparamresource?id=50")
	...
	// a bad status
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/multiparamresource")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		body, err := io.ReadAll(rec.Body)
		if assert.NoError(t, err) {
	...
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/multiparamresource?id=500")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		body, err := io.ReadAll(rec.Body)
		if assert.NoError(t, err) {
			assert.Contains(t, string(body), "parameter \"id\"")
	...
	// return a bad status
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/multiparamresource?id=abc&id2=1")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		body, err := io.ReadAll(rec.Body)
		if assert.NoError(t, err) {
			assert.Contains(t, string(body), "parameter \"id\"")
...
func TestOapiRequestValidatorWithOptions(t *testing.T) {
	...
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/protected_resource")
	...
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/protected_resource2")
	...
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/protected_resource_401")
		assert.Equal(t, http.StatusUnauthorized, rec.Code)
...
func testRequestValidatorBasicFunctions(t *testing.T, r *chi.Mux) {
	...
	{
		rec := doGet(t, r, "http://not.gerhardwagner.ai/resource")
	...
	// Let's send a good request, it should pass
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/resource")
		assert.Equal(t, http.StatusOK, rec.Code)
		assert.True(t, called, "Handler should have been called")
		called = false
	}
	...
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/resource?id=500")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}
	...
	{
		rec := doGet(t, r, "http://gerhardwagner.ai/resource?id=foo")
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		assert.False(t, called, "Handler should not have been called")
		called = false
	}
	...
	// Send a good request body
	{
		...
		rec := doPost(t, r, "http://gerhardwagner.ai/resource", body)
		assert.Equal(t, http.StatusNoContent, rec.Code)
		assert.True(t, called, "Handler should have been called")
		called = false
	}
